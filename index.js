...
var account1 = new SteamBotAccountManager(
  mainClient,
  mainCommunity,
  mainManager,
  account1Credentials
);
var account2 = new SteamBotAccountManager(
  botClient,
  botCommunity,
  botManager,
  account2Credentials
...
try {
  account1.getClient().logOn({
    ...
    twoFactorCode: SteamTotp.generateAuthCode(
      account1Credentials.getSharedSecret()
    ),
  });
  ...
  account2.getClient().logOn({
    ...
    twoFactorCode: SteamTotp.generateAuthCode(
      account2Credentials.getSharedSecret()
    ),
  });
} catch {
  console.log(
    "Something went wrong with the signon process. If a steam guard message appeared try again in 1 minute If not " +
...
account1.getTradeOfferBot().on("newOffer", async (offerResponse) => {
  ...
  if (offer.isSafeTrade()) {
    account1.printMessage(
      "Confirmed this is a safe trade, accepting the offer."
    );
...
account2.getTradeOfferBot().on("newOffer", async (offerResponse) => {
  ...
  if (offer.isSafeTrade()) {
    ...
    account1.printMessage("Sending the offer with a " + newItem.getName());
    await account2.sendTradeOffer(
      account1Credentials.getTradelink(),
      SECURITY_CODE.toString(),
      [newItem],
      MAX_RETRIES
    );
